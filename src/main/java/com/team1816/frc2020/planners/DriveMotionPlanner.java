...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public double getMaxRotationSpeed() {
        ...
        double normalizedProgress =
            mCurrentTrajectory.getProgress() / currentTrajectoryLength;
        double scalar = 0.0;
        if (kStartPoint <= normalizedProgress && normalizedProgress <= kEndPoint) {
            if (normalizedProgress <= kPivotPoint) {
                ...
                scalar =
                    1.0 -
                    ((normalizedProgress - kPivotPoint) / (kEndPoint - kPivotPoint));
            }
        }
    ...
    protected Translation2d updatePurePursuit(Pose2d current_state) {
        ...
        SmartDashboard.putNumber(
            "Path Velocity",
            lookahead_state.velocity() / Constants.kPathFollowingMaxVel
        );

        Translation2d lookaheadTranslation = new Translation2d(
            current_state.getTranslation(),
        ...
        if (
            ...
            mSetpoint.t() > (currentTrajectoryLength / 2.0)
        ) {
    ...
    public Translation2d update(double timestamp, Pose2d current_state) {
        ...
        current_state =
            current_state.transformBy(Pose2d.fromTranslation(followingCenter));
        ...
        double reverseDistance = distance(
            current_state,
            previewQuantity - searchStepSize
        );
