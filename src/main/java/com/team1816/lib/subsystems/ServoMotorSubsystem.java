...
public abstract class ServoMotorSubsystem extends Subsystem {
    ...
    protected ServoMotorSubsystem(final ServoMotorSubsystemConstants constants) {
        ...
        mMaster =
            (TalonSRX) CtreMotorFactory.createDefaultTalon(
        ...
        mForwardSoftLimitTicks =
            (int) (
                (mConstants.kMaxUnitsLimit - mConstants.kHomePosition) *
        ...
        mReverseSoftLimitTicks =
            (int) (
                (mConstants.kMinUnitsLimit - mConstants.kHomePosition) *
                mConstants.kTicksPerUnitDistance
            );
        MotorUtil.checkError(
        ...
        MotorUtil.checkError(
            mMaster.configMaxIntegralAccumulator(
                kMotionProfileSlot,
                mConstants.kMaxIntegralAccumulator,
                Constants.kLongCANTimeoutMs
            ),
        ...
        MotorUtil.checkError(
            mMaster.configMaxIntegralAccumulator(
                kPositionPIDSlot,
                mConstants.kPositionMaxIntegralAccumulator,
                Constants.kLongCANTimeoutMs
            ),
            mConstants.kName + ": Could not set max integral: "
        );
        ...
        MotorUtil.checkError(
            mMaster.configContinuousCurrentLimit(
                mConstants.kContinuousCurrentLimit,
                Constants.kLongCANTimeoutMs
            ),
        ...
        MotorUtil.checkError(
            mMaster.configPeakCurrentLimit(
                mConstants.kPeakCurrentLimit,
                Constants.kLongCANTimeoutMs
            ),
        ...
        MotorUtil.checkError(
            mMaster.configPeakCurrentDuration(
                mConstants.kPeakCurrentDuration,
                Constants.kLongCANTimeoutMs
            ),
        ...
        for (int i = 0; i < mSlaves.length; ++i) {
            mSlaves[i] =
                (TalonSRX) CtreMotorFactory.createPermanentSlaveTalon(
                    mConstants.kSlaveConstants[i].id,
                    false,
                    mMaster
                );
    ...
    public synchronized void readPeriodicInputs() {
        ...
        if (mFaults.hasAnyFault()) {
            DriverStation.reportError(
                mConstants.kName + ": Talon Fault! " + mFaults.toString(),
                false
            );
        ...
        if (mConstants.kRecoverPositionOnReset) {
            mPeriodicIO.absolute_pulse_position =
                mMaster.getSensorCollection().getPulseWidthPosition();
            mPeriodicIO.absolute_pulse_position_modded =
                mPeriodicIO.absolute_pulse_position % 4096;
            ...
            double estimated_pulse_pos =
                ...
                mPeriodicIO.absolute_pulse_offset;
    ...
    protected int getAbsoluteEncoderRawPosition(int pulseWidthPosition) {
        ...
        return (
            ...
            (abs_raw_with_rollover < 0 ? abs_raw_with_rollover + 4096 : 0)
        );
    }
            ...
            new Loop() {
                ...
                public void onLoop(double timestamp) {
                    if (mPeriodicIO.reset_occured) {
                        System.out.println(
                            mConstants.kName +
                            ": Master Talon reset occurred; resetting frame rates."
                        );
                        mMaster.setStatusFramePeriod(
                        ...
                        if (mConstants.kRecoverPositionOnReset) {
                            mMaster.setSelectedSensorPosition(
                                estimateSensorPositionFromAbsolute(),
                                0,
    ...
    public synchronized double getSetpoint() {
        return (
            ...
            ? ticksToUnits(mPeriodicIO.demand)
            : Double.NaN;
    }
    ...
    public synchronized void setGoalMotionProfiling(
        ...
        MotionState cur_state = new MotionState(
            mPeriodicIO.timestamp,
            mPeriodicIO.position_units,
            ticksPer100msToUnitsPerSecond(mPeriodicIO.velocity_ticks_per_100ms),
            0.0
        );
        Setpoint setpoint = mSetpointGenerator.getSetpoint(
            mMotionProfileConstraints,
            goal,
            cur_state,
        ...
        mPeriodicIO.demand =
            constrainTicks(homeAwareUnitsToTicks(setpoint.motion_state.pos()));
        mPeriodicIO.feedforward =
            (
                (feedforward_v + setpoint.motion_state.vel()) *
                mConstants.kKf +
                setpoint.motion_state.acc() *
                mConstants.kKa
            ) /
            1023.0;
    }
    ...
    public synchronized double getPredictedPositionUnits(double lookahead_secs) {
        ...
        double predicted_units = ticksToHomedUnits(
            mPeriodicIO.active_trajectory_position +
            lookahead_secs *
            mPeriodicIO.active_trajectory_velocity +
            0.5 *
    ...
    public int estimateSensorPositionFromAbsolute() {
        int estimated_pulse_pos =
            (mPeriodicIO.encoder_wraps * 4096) +
            mPeriodicIO.absolute_pulse_position_modded;
        int estimate_position_ticks =
            (mConstants.kMasterConstants.invert_sensor_phase ? -1 : 1) *
